...
- `variant_ep8full` — **ep8full** () = lora-opti + jybsuper **PR#21** squash (, EP8 cuda-graph fix) + **PR#22** all-in-one LoRA-GEMM as a **clean 3-way merge** (restores PR#21's  dispatch that the earlier PR#22 *squash* had clobbered — root cause of the  garbage).
...
Profiles:  (cuda-graph ON, 8 TP ranks) +  (cuda-graph OFF, kernel structure, rank 0+4).; contents: bench, prompts, traces (graph-on=8, graph-off=2)
