# `kimi-reg-ed8d3598c3-20260602_212619`
...
- `variant_ep8` — **LoRA candidate** on `ed8d3598c3`, override: `ep8` (drop `cell.md` to describe the exact envs); contents: bench, prompts, traces (graph-on=4, graph-off=4)
...
|---|---|---|---|
| `variant_ep8` | OK | — | coherent |

## Traces — separate GitHub Release

All `.trace.json.gz` files for this run are attached to the release **`kimi-reg-ed8d3598c3-20260602_212619`** in `jybsuper/lora-traces`. Each cell has its own tarball (`<cell>_traces.tar.gz`) containing `traces/graph_on/` (all TP ranks across both pods, bs64 unless noted) and `traces/graph_off/` (rank-0 only, bs64).

Download one cell's traces:

```bash
gh release download kimi-reg-ed8d3598c3-20260602_212619 --repo jybsuper/lora-traces --pattern '<cell>_traces.tar.gz'
...
```bash
gh release download kimi-reg-ed8d3598c3-20260602_212619 --repo jybsuper/lora-traces
